...
Failed tests automatically capture screenshots to `reports/screenshots/`.
"""
...
def driver(settings: Settings, request: pytest.FixtureRequest) -> WebDriver:
    ...
    try:
        ...
        if rep_call and rep_call.failed:
            ...
            test_id = request.node.nodeid.replace("::", "__").replace("/", "_").replace("\\", "_")
            screenshot_path = SCREENSHOTS_DIR / f"{test_id}_{timestamp}.png"
